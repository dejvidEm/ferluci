  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (unuploadedImages.length > 0) {
        throw new Error('Pred odoslaním prosím nahrajte všetky obrázky')
      }
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Nepodarilo sa vytvoriť vozidlo')
      }

      setSuccess({ id: data.id, slug: data.slug })
      setLoading(false)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Nepodarilo sa vytvoriť vozidlo')
      setLoading(false)
    }
  }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-green-500/10 border border-green-500/50 rounded-lg p-6 mb-6">
            <h2 className="text-2xl font-bold text-green-400 mb-2">
              Vozidlo bolo úspešne vytvorené!
            </h2>
            <p className="text-gray-300 mb-4">
              Vaše vozidlo bolo pridané do systému.
            </p>
            <div className="flex flex-wrap gap-4">
              <Button asChild>
                <Link href={`/ponuka/${success.id}`}>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Pridať nové vozidlo</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 md:space-y-8">
          ...
          <div className="flex gap-4 sticky bottom-0 bg-[#121212] pt-4 pb-2 -mx-3 sm:-mx-4 md:-mx-8 px-3 sm:px-4 md:px-8 border-t border-white/10">
            <Button type="submit" disabled={loading} className="flex-1 text-sm sm:text-base">
              {loading ? 'Vytváram vozidlo...' : 'Vytvoriť vozidlo'}
            </Button>
          </div>
        </form>
      </div>
