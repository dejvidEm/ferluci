...
interface VehicleInput {
  ...
  description: string
  features: string[]
  imageAssetIds: string[] // Array of Sanity asset IDs in order
  featured: boolean
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build images array with Sanity image references
    // Each array item needs a unique _key property
    const images = body.imageAssetIds.map((assetId, index) => ({
      _type: 'image' as const,
      _key: `image-${index}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
      asset: {
    ...
    const result = await adminClient.create(vehicleDoc)
